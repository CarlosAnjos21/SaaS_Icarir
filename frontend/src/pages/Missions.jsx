...
export default function Missions() {
  ...
  const handleCompleteStep = (stepId) => {
    if (!currentClientId) return alert("Selecione um participante!");
    ...
    const updatedMissions = missoes.map((mission) => {
      if (mission.id === selectedMission.id) {
        const updatedSteps = mission.steps.map((step) => {
          if (
            step.id === stepId &&
            !step.completedBy.includes(parseInt(currentClientId))
          ) {
            return {
              ...step,
              completedBy: [...step.completedBy, parseInt(currentClientId)],
            };
          }
          return step;
        });
  ...
  const isMissionComplete = (mission) =>
    ...
    mission.steps.every(
      (step) =>
        Array.isArray(step.completedBy) &&
        step.completedBy.includes(parseInt(currentClientId))
    );
  ...
  if (loading) return <p className="text-center mt-20">Carregando dados...</p>;
  if (error)
            ...
            <div className="mb-6">
              ...
              <ul className="space-y-2">
                {selectedMission.steps.map((step) => {
                  const isDone = step.completedBy.includes(
