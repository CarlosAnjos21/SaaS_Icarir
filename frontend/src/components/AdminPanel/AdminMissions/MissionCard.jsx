...
const MissionCard = ({ mission, onEdit, onDelete, onManageParticipants }) => {
    ...
    const rawImage = mission.imageUrl || mission.foto_url || mission.image;

    const isValidImage = 
        rawImage && 
        typeof rawImage === "string" &&
        rawImage.startsWith("http") &&
        !rawImage.includes("exemplo.com");

    const displayImage = isValidImage && !imgError ? rawImage : null;
    ...
    if ((!backendTotal || backendTotal === 0) && mission._raw) {
        ...
        if ((!backendTotal || backendTotal === 0) && Array.isArray(mission._raw.tarefas)) {
            backendTotal = mission._raw.tarefas.reduce(
                (acc, t) => acc + (Number(t.pontos || t.points || 0) || 0),
                0
            );
        }
    }

    const stepsArray = Array.isArray(mission.steps)
        ? mission.steps
        : (Array.isArray(mission.tarefas) ? mission.tarefas : []);

    const calculatedPoints = stepsArray.reduce(
        (s, it) => s + (Number(it.points || it.pontos || 0) || 0),
        0
    );
    ...
    const displayPoints =
        backendTotal > 0
            ...
            : (calculatedPoints > 0
                ? calculatedPoints
                : Number(mission.points || mission.pontos || 0));
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all group overflow-hidden flex flex-col h-full relative">
            ...
            <div className="p-5 flex-1 flex flex-col">
                ...
                <div className="flex justify-between items-start mb-2">
                    <h3 
                        className="text-lg font-bold text-gray-800 line-clamp-1 group-hover:text-[#394C97] transition-colors" 
                        title={mission.title}
                    >
                        {mission.title}
                    </h3>
                ...
                <div className="pt-4 border-t border-gray-100 flex items-center justify-between mt-auto">
                    <span className="flex items-center gap-1 text-[10px] text-gray-400 font-medium">
                        ...
                        {mission.expirationDate 
                            ...
                            : (mission.data_fim 
                                ? new Date(mission.data_fim).toLocaleDateString() 
                                : 'Sem data')}
                    </span>
