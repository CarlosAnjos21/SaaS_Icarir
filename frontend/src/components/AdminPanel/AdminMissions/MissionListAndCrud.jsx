...
// API
import { 
    fetchMissions, // Usa a rota pública ou de admin dependendo da config, idealmente fetchMissionsAdmin se existir separada
    createMission, 
    updateMission, 
    deleteMissionApi, 
    createTask, 
...
function normalizeMission(m) {
    ...
    // CORREÇÃO CRÍTICA: Mapeamento da foto_url
    ...
    return {
        ...
        imageUrl: imgUrl, // Campo que o Modal e o Card usam
        foto_url: imgUrl, // Mantém compatibilidade com banco
        _raw: m,
    };
}
    ...
    const handleModalOpen = (missionToEdit = null) => {
        if (missionToEdit) {
            ...
            setNewMission({
                ...
                points: m.points || 0,
                expirationDate: m.data_fim ? String(m.data_fim).slice(0,10) : (m.expirationDate || ""),
                imageUrl: m.foto_url || m.imageUrl || "", // Garante que a imagem venha para a edição
                quiz: normalizeQuizToUI(m.quiz),
            });
        } else {
            setIsEditing(false);
    ...
    const handleSaveMission = async () => {
        ...
        try {
            const payload = {
                ...
                ativa: true,
                missao_anterior_id: null,
                // CORREÇÃO: Garante o envio da URL da foto
            ...
            if (isEditing) {
                ...
                // Sincronização de Tarefas (Mantida)
