...
const MissionModal = ({
  ...
  const totalMissionPoints = useMemo(() => {
    const fromLinked =
      Array.isArray(linkedTasks) && linkedTasks.length > 0
        ? linkedTasks.reduce(
            (acc, task) => acc + (Number(task.pontos || task.points || 0) || 0),
            0,
          )
        : 0;
    ...
    const fromSteps = Array.isArray(newMission?.steps)
      ? newMission.steps.reduce(
          (acc, s) => acc + (Number(s.points || s.pontos || 0) || 0),
          0,
        )
      : 0;
    ...
    try {
      ...
      const missionTasks = Array.isArray(allTasks)
        ? allTasks.filter((t) => {
            ...
            return (
              taskMissionId &&
              Number(taskMissionId) === targetMissionId &&
              isActive
            );
          })
        : [];
  ...
  const onSaveWrapper = async () => {
    ...
    console.log(
      "Nota: O campo 'pontos' não será salvo na tabela 'missoes', pois é derivado.",
    );
    console.groupEnd();
      ...
      >
        ...
        <div className="px-8 py-6 border-b border-gray-100 bg-gray-50/50 rounded-t-2xl">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Configure os detalhes e organize a jornada do usuário.
          </p>
        </div>
            ...
            <div className="lg:col-span-2 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewMission({ ...newMission, titulo: e.target.value })
                  }
                  disabled={isLoading}
                />
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      value={
                        newMission.expirationDate
                          ? newMission.expirationDate.split("T")[0]
                          : ""
                      }
                      onChange={(e) =>
                        setNewMission({
            ...
            {loadingTasks ? (
                      ...
                      <div className="bg-white border border-gray-200 rounded-xl p-3 flex items-center gap-3 shadow-sm hover:border-[#394C97] transition-colors cursor-grab active:cursor-grabbing group relative pr-12">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-0.5">
                            ...
                            {task.tipo === "social" && (
                              <span className="bg-purple-50 text-purple-600 text-[10px] px-1.5 py-0.5 rounded font-bold flex items-center gap-1">
                                <Camera size={10} /> Social
                              </span>
                            )}
                            {task.tipo === "conhecimento" && (
                              <span className="bg-indigo-50 text-indigo-600 text-[10px] px-1.5 py-0.5 rounded font-bold flex items-center gap-1">
                                <CheckCircle size={10} /> Quiz
                              </span>
                            )}
                            {(!task.tipo || task.tipo === "padrao") && (
              ...
              <div className="text-center py-8 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Crie tarefas na aba "Tarefas" e vincule a esta missão.
                </p>
              </div>
            )}
          </section>
        ...
        <div className="p-6 border-t border-gray-100 bg-gray-50 flex justify-end gap-3 rounded-b-2xl sticky bottom-0 z-20">
          <button
            onClick={handleModalClose}
            disabled={isLoading}
            className="px-6 py-2.5 rounded-xl text-gray-600 font-bold text-xs hover:bg-gray-200 transition-colors uppercase tracking-wide"
          >
            Cancelar
          </button>
          <button
            ...
            className="px-8 py-2.5 rounded-xl bg-[#394C97] text-white font-bold shadow-lg hover:bg-[#2a385f] transition-all flex items-center gap-2 text-xs uppercase tracking-wide"
          >
            {isLoading || uploading ? (
