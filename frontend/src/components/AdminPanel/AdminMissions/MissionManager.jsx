...
function normalizeQuizToUI(quiz) {
  ...
  if (quiz.question || quiz.options) {
    const opts = Array.isArray(quiz.options)
      ? quiz.options
      : INITIAL_QUIZ_STATE.options;
    const idx = typeof quiz.correctIndex === "number" ? quiz.correctIndex : 0;
    return { question: quiz.question || "", options: opts, correctIndex: idx };
  }
  const firstQuestion = Array.isArray(quiz.perguntas)
    ? quiz.perguntas[0]
    : null;
  const opts = Array.isArray(firstQuestion?.opcoes)
    ? firstQuestion.opcoes
    : INITIAL_QUIZ_STATE.options;
  const idx = firstQuestion?.resposta_correta
    ? opts.findIndex((opt) => opt === firstQuestion.resposta_correta)
    : -1;
  return {
...
function normalizeMission(m) {
  ...
  const totalPoints = steps.reduce(
  ...
  return {
    ...
    totalPoints: Number(
      m.totalPoints ?? totalPoints ?? m.pontos ?? m.points ?? 0,
    ),
    preco: m.preco != null ? Number(m.preco) : null,
    vagas_disponiveis:
      m.vagas_disponiveis != null ? Number(m.vagas_disponiveis) : null,
    expirationDate: m.data_fim
      ? String(m.data_fim).slice(0, 10)
      : m.expirationDate || "",
    ...
    try {
      const payload = {
        ...
        titulo:
          ...
          s.title ||
          (s.description
            ? String(s.description).length > 30
              ? String(s.description).slice(0, 30) + "..."
...
const MissionManager = () => {
  ...
  const loadMissions = useCallback(async () => {
    ...
    try {
      ...
      setError(
        `Falha ao carregar missões: ${err.message || "Erro de conexão"}`,
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSaveMission = async () => {
    if (
      !(newMission.titulo || newMission.title) ||
      !(newMission.destino || newMission.city)
    ) {
    ...
    const startDate =
      newMission.data_inicio || newMission.startDate || todayIso;
    ...
    try {
      const payload = {
        ...
        data_fim: endDate,
        preco:
          newMission.preco != null ? newMission.preco : newMission.points || 0,
        vagas_disponiveis:
          newMission.vagas_disponiveis != null
            ? newMission.vagas_disponiveis
            : 0,
        ativa: true,
        missao_anterior_id: null,
      ...
      if (isEditing) {
        ...
        const targetId = updated?.id ?? editingId;

        const originalMission = missions.find((m) => m.id === editingId);
        const originalStepIds = (originalMission?.steps || [])
          .filter((s) => s.id)
          .map((s) => s.id);
        const currentStepIds = (newMission.steps || [])
          .filter((s) => s.id)
          .map((s) => s.id);
        const toDeleteIds = originalStepIds.filter(
          (id) => !currentStepIds.includes(id),
        );

        await syncTasksForMission(targetId, newMission.steps || []);

        if (toDeleteIds.length > 0) {
          await Promise.all(
            toDeleteIds.map((id) =>
              deleteTask(id).catch((e) => console.error(e)),
            ),
          );
        }
      } else {
        ...
        await syncTasksForMission(createdId, newMission.steps || []);
      }
