...
const getRoleBadge = (role) => {
  switch (role) {
    ...
    case "validador":
      return (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-700 border border-amber-200 uppercase tracking-wide">
          <CheckCircle size={10} /> Validador
        </span>
      );
    case "participante":
    default:
...
const getStatusBadge = (active) => {
  return active ? (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-medium bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20">
      <span className="w-1.5 h-1.5 rounded-full bg-emerald-600"></span> Ativo
    </span>
  ) : (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-medium bg-gray-50 text-gray-600 ring-1 ring-inset ring-gray-500/20">
      <span className="w-1.5 h-1.5 rounded-full bg-gray-500"></span> Inativo
    </span>
  );
};
        ...
        <tbody className="bg-white divide-y divide-gray-50">
          {users.map((user, index) => (
            ...
            >
              ...
              {showPoints && (
                <td className="px-4 py-2 whitespace-nowrap">
                  <span className="font-mono font-bold text-[#394C97] bg-blue-50 px-1.5 py-0.5 rounded text-xs">
                    {user.pontos}
                  </span>
                </td>
              )}
              ...
              <td className="px-4 py-2 whitespace-nowrap text-[10px] text-gray-400">
                {user.data_criacao
  ...
  const handleModalOpen = (userToEdit = null) => {
    if (userToEdit) {
      ...
      setCurrentUser({
        ...
        ...userToEdit,
        senha: "",
        pontos: Number(userToEdit.pontos || 0),
      });
    } else {
      setIsEditing(false);
  ...
  const handleSaveUser = async () => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        setUsers((prev) =>
          prev.map((u) => (u.id === currentUser.id ? userToUpdate : u)),
        );
      } else {
        ...
        <div className="max-w-7xl mx-auto px-6 h-full flex items-center pb-10 md:translate-y-2 relative z-10">
          ...
          >
            <div className="p-3 bg-white/10 rounded-2xl backdrop-blur-md border border-white/10 shadow-xl ring-1 ring-white/20">
              <Users className="w-6 h-6 text-[#FE5900]" />
            </div>
            <div>
              <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight">
                Gestão de Usuários
              </h1>
              <p className="text-blue-100/90 text-sm md:text-base mt-1 font-light">
                Controle de acesso e membros da plataforma
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-24 relative z-20">
        <div className="flex flex-col md:flex-row justify-between items-center gap-3 mb-6">
          <div className="flex bg-white p-1 rounded-lg shadow-md border border-gray-100">
            <button
              onClick={() => setActiveCategory("participante")}
              className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${activeCategory === "participante" ? "bg-blue-50 text-blue-700 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
            >
              Participantes
            </button>
            <button
              onClick={() => setActiveCategory("admin")}
              className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${activeCategory === "admin" ? "bg-rose-50 text-rose-700 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
            >
              Administrativo
            </button>
          </div>
          <div className="flex items-center gap-3 w-full md:w-auto">
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="flex-1 md:flex-none w-full md:w-64 bg-white p-1.5 rounded-lg shadow-md border border-gray-100 flex items-center gap-2 px-3"
            >
            ...
            <motion.button
              ...
              className="bg-[#FE5900] text-white px-4 py-2 rounded-lg shadow-md hover:bg-[#e04f00] hover:shadow-orange-500/20 transition-all flex items-center justify-center gap-1.5 font-bold text-xs tracking-wide transform hover:-translate-y-0.5 whitespace-nowrap"
            >
        ...
        {error && users.length === 0 && (
          <motion.div
            ...
            className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg flex items-center justify-between gap-3 mb-4 shadow-sm text-sm"
          >
        ...
        >
          <UserTable
            ...
            isLoading={isSaving}
            showPoints={activeCategory === "participante"}
          />
        </motion.div>

        <div className="mt-3 text-right text-[10px] font-medium text-gray-400">
          Total nesta categoria: {filteredUsers.length}
        </div>
