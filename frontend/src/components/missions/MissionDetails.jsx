...
const getTaskIcon = (taskName = "", category = "", isCompleted) => {
  ...
  if (nameLower.includes("envio") || nameLower.includes("documento"))
    return Upload;
  if (nameLower.includes("formulário") || nameLower.includes("feedback"))
    return Edit;
  if (nameLower.includes("linkedin") || nameLower.includes("comentar"))
    return MessageSquare;

  const catLower = (
    typeof category === "string" ? category : category?.nome || ""
  ).toLowerCase();
...
const TaskItem = ({ task, onClick, isLocked, isCompleted }) => {
  ...
  if (isCompleted) {
    ...
    containerClasses =
      "border-gray-100 opacity-70 cursor-not-allowed bg-gray-50";
    ...
    >
      <div className="flex items-start space-x-4 flex-1 min-w-0">
        ...
        <div className="min-w-0 overflow-hidden pt-0.5">
          ...
          <p
            className={`text-sm truncate mt-0.5 ${isCompleted || isLocked ? "text-gray-400" : "text-gray-600"}`}
          >
            {description}
          </p>
        </div>
...
const TaskDetailsModal = ({
    ...
    const loadData = async () => {
      ...
      try {
        const res = await api.get(
          `/missions/${initialTask.missao_id}/tasks/${initialTask.id}`,
        );
  ...
  const questions = useMemo(() => {
    ...
    if (task.quiz) {
      if (
        Array.isArray(task.quiz.perguntas) &&
        task.quiz.perguntas.length > 0
      ) {
        found = task.quiz.perguntas;
      } else if (
        Array.isArray(task.quiz.questions) &&
        task.quiz.questions.length > 0
      ) {
  ...
  const handleFileChange = (e) => {
    ...
    if (f) {
      if (f.size > 25 * 1024 * 1024)
        return setError("Arquivo muito grande (Max 25MB)");
      setFile(f);
      setError(null);
    }
  };
            ...
            {questions.map((q, idx) => (
              <div key={q.id} className="space-y-2">
                ...
                {q.opcoes.map((opt, optIdx) => (
                  <label
                    key={optIdx}
                    className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all ${quizAnswers[q.id] === opt ? "border-indigo-500 bg-indigo-50" : "border-gray-200 hover:bg-gray-50"}`}
                  >
                    <input
                      type="radio"
                      name={`q-${q.id}`}
                    ...
                    />
                    <div
                      className={`w-4 h-4 rounded-full border flex items-center justify-center ${quizAnswers[q.id] === opt ? "border-indigo-600" : "border-gray-400"}`}
                    >
      ...
      return (
        <div className="text-center py-8">
          ...
          <p className="text-xs text-gray-400 mt-1 max-w-xs mx-auto">
            O sistema não conseguiu identificar a lista de perguntas no objeto
            da tarefa.
          </p>
        </div>
      );
      ...
      return (
        <div className="space-y-6">
          <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 text-blue-900">
            <h4 className="font-bold flex items-center gap-2 text-sm">
              <FileText size={16} /> Envio de Arquivo
            </h4>
      ...
      <div className="space-y-6">
        <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
          ...
          <p className="text-sm text-gray-600">
            {task.descricao || "Realize a atividade."}
          </p>
        </div>
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl w-full max-w-lg shadow-2xl relative overflow-hidden max-h-[90vh] flex flex-col"
      ...
      >
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar">
          {error && (
            <div className="mb-4 bg-red-50 text-red-600 p-3 rounded-lg text-xs font-bold flex gap-2">
...
export default function MissionDetails({
  ...
  const fetchDetails = useCallback(async () => {
    ...
    try {
      const endpoint = readOnly
        ? `/missions/${initialMissionData.id}`
        : `/missions/${initialMissionData.id}/full`;
  ...
  const handleLeave = async () => {
    ...
    if (
      !window.confirm(
        "Tem certeza que deseja sair desta missão? Seu progresso será perdido.",
      )
    )
      return;
    setLeaving(true);
  ...
  const handleTaskClick = (task) => {
    ...
    const participationStatus = String(
      currentData.status_participacao || "",
    ).toLowerCase();
  ...
  const participationStatus = String(
    mission?.status_participacao || mission?.statusParticipacao || "",
  ).toLowerCase();
  ...
  const totalPoints = useMemo(() => {
    ...
    if (items.length === 0)
      return Number(mission.pontos || mission.points || 0);
    return items.reduce(
      (acc, item) => acc + (Number(item.pontos || item.points || 0) || 0),
      0,
    );
  }, [mission, tarefas]);

  if (loading && !fullMissionData && !initialMissionData)
    return (
    ...
    activeTab === "Todas"
      ...
      : tarefas.filter(
          (t) =>
            (typeof t.categoria === "string"
              ? t.categoria
              : t.categoria?.nome || "Geral") === activeTab,
        );

  return (
    <div
      className={`bg-gray-50 ${readOnly ? "" : "p-4 sm:p-6 md:p-8 min-h-screen pb-24"}`}
    >
      <div className={readOnly ? "" : "max-w-5xl mx-auto"}>
              ...
              <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 flex items-center justify-between gap-4 animate-fade-in">
                ...
                <div className="flex-1 max-w-sm">
                  <div className="flex justify-between text-xs font-semibold text-gray-500 mb-1.5">
                    <span>Progresso</span>
                    <span>
                      {Math.round(progressPercentage)}% ({completedCount}/
                      {totalTasks})
                    </span>
                  </div>
        ...
        <div
          className={`grid grid-cols-1 ${readOnly ? "" : "lg:grid-cols-3"} gap-8`}
        >
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
                <h3 className="font-bold text-gray-800 mb-4 flex items-center">
                  <Trophy className="w-5 h-5 text-yellow-500 mr-2" /> Ranking
                </h3>
                    ...
                    {ranking.map((user, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span
                            className={`w-5 h-5 flex items-center justify-center rounded-full text-xs font-bold ${idx === 0 ? "bg-yellow-100 text-yellow-700" : "bg-gray-100 text-gray-500"}`}
                          >
                            {idx + 1}
                          </span>
                          <span
                            className={`truncate max-w-[120px] ${user.isCurrentUser ? "font-bold text-[#394C97]" : "text-gray-600"}`}
                          >
                            {user.name}
                          </span>
                        </div>
