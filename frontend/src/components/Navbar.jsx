...
export default function Navbar() {
  ...
  const navbarClasses = `w-full px-8 py-1 flex justify-between items-center fixed top-0 left-0 z-50 backdrop-blur-md transition-all duration-500 ${
    isTransparent
  ...
  const navLinks = [
    ...
    isAuthenticated &&
      user?.role !== "admin" && { name: "Missão", path: "/missions" },
    isAuthenticated &&
      user?.role !== "admin" && { name: "Classificação", path: "/ranking" },
  ...
  return (
    <nav className={navbarClasses}>
      {/* LOGO */}
      <Link to="/" className="flex items-center gap-2 z-50 pl-14">
        <img
          src={logoIcarir}
          alt="Logo ICARIR"
      ...
      {/* MENU DESKTOP */}
      <ul className="hidden md:flex gap-8 text-base font-medium tracking-wide pr-20">
          ...
          {isAuthenticated ? (
            ...
            >
              {user?.foto_url ? (
                ...
                <div
                  className={`h-9 w-9 rounded-full flex items-center justify-center border-2 font-bold text-xs ${
                    isTransparent
            ...
            <div className="flex items-center gap-3">
              ...
              <Link to="/login" title="Acesso Administrativo">
                <button
                  className={`hover:text-red-500 transition focus:outline-none flex items-center ${isTransparent ? "text-white/80" : "text-[#394C97]"}`}
                >
          ...
          {menuOpen && isAuthenticated && user && (
            <div className="absolute right-0 mt-3 w-56 bg-white text-[#394C97] rounded-lg shadow-xl border border-gray-100 overflow-hidden z-50 animate-fade-in">
              <div className="px-4 py-3 border-b border-gray-100 mb-1 bg-gray-50">
                ...
                <p className="text-sm font-bold truncate text-[#FE5900]">
                  {user.nome}
                </p>
              </div>
                  ...
                  return (
                    <li key={index}>
                      {item.isLogout ? (
                        ...
                        >
                          <Icon
                            className={`h-4 w-4 ${item.rotateIcon ? "rotate-180" : ""}`}
                          />
                          {item.name}
                        </Link>
                      )}
            ...
            {isAuthenticated && user && (
              <>
                <div className="border-t border-gray-100 mt-2 pt-2 px-6 pb-2">
                  ...
                  <p className="text-sm font-bold text-[#FE5900]">
                    {user.nome}
                  </p>
                </div>
                {userMenuItems.map((item, index) => {
